...
```{r cars-plot, dev='png',out.width="500px", out.height="300px"}
...
### We could figure stratify these tables based on gene types to see if the values differ between subgroups.

### Potential analysis: calculate pointwise mutual information (pmi) between each flavor combination.

### This is a measure of association for binary events, capturing the probability they are observed together relative to them being independent:

$$
pmi = \frac{p(flavor_1 | flavor_2)}{p(flavor_1)} \\
$$

### We could either calculate this for each individual for their 120 minutes and see if the value is different for people with different genes or calculate across all individuals at each time point to see if it is time-varying on a group level.
